...
def capture_photo():
    try:
        ...
        captures_dir = f"/home/pi_zero_1/webserver/captures/"
        metafile_path = f"/home/pi_zero_1/webserver/captures/{filename}_metadata.json"
       ...
       # bytes_metadata = piexif.dump(metadata)
        ...
        pil_image.save(f'{captures_dir}{filename}_rgb.jpg')
        r.save(f"{captures_dir}{filename}_red.jpg")
        g.save(f"{captures_dir}{filename}_green.jpg")
        b.save(f"{captures_dir}{filename}_blue.jpg")
        print('start compression')
        # send everything to a zip file for transfer to main pi
        zip_data = io.BytesIO()
        with ZipFile(zip_data, 'w') as zipf:
            zipf.write(metafile_path, arcname=f'{filename}_vis_metadata.json')
            zipf.write(f'{captures_dir}{filename}_rgb.jpg', arcname=f'{filename}_rgb.jpg')
            zipf.write(f'{captures_dir}{filename}_red.jpg', arcname=f'{filename}_red.jpg')
            zipf.write(f'{captures_dir}{filename}_green.jpg', arcname=f'{filename}_green.jpg')
            zipf.write(f'{captures_dir}{filename}_blue.jpg', arcname=f'{filename}_blue.jpg')
        zip_data.seek(0)
